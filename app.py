...
def add_vintage_mob(df: pd.DataFrame) -> pd.DataFrame:
    """(Kept for other calcs) Adds quarterly Vintage (YYYYQx) and monthly MOB."""
    df = df.copy()
...
def run_integrity_checks(df: pd.DataFrame, dpd_threshold: int, gap_days: int = 120, after_mat_tol_days: int = 31):
    ...
    # QOB aggregation for denominator sanity (Option 1)
    dfd_q = add_qob(dfd)
    ...
    # Raw default rate non-monotone by QOB
...
# ----------------------------
# Vintage default summary helper (Cum_PD, Obs_Time, annualized rate)
# ----------------------------
...
if uploaded:
    ...
    try:
        ...
        if df_plot_any.empty:
            ...
            st.download_button('Download curves (CSV)', csv_series, 'vintage_curves_qob.csv','text/csv')

            if fig is not None:
                buf = BytesIO()
                fig.savefig(buf, format='png', bbox_inches='tight', dpi=160)
                st.download_button('Download chart (PNG)', buf.getvalue(), 'vintage_curves_qob.png', 'image/png')
    except Exception as e:
